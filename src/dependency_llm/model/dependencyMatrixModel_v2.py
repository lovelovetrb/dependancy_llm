...
class DependencyMatrixModel(nn.Module):
    ...
    def forward(
        ...
        pred = torch.tensor([], requires_grad=True).to("cuda")
        for dep_origin_datas in dep_data:
            pred_deps_data = torch.tensor([]).to("cuda")

            for scope_token_data in dep_origin_datas:
                scope = scope_token_data["scope"]
                ...
                score = self.last_layer(sequence_output)
                score = score[0]  # (batch_size, 1)
                pred_deps_data = torch.cat([pred_deps_data, score], dim=0)

            deps_index = torch.argmax(pred_deps_data, dim=0)
            # deps_index以外のindexを0にする
            # DEBUG
            # print(pred_deps_data)
            pred_deps_data = torch.zeros_like(pred_deps_data).scatter_(
                0, deps_index.unsqueeze(0), 1
            )

            pred = torch.cat([pred, pred_deps_data], dim=0)

        pred = pred.unsqueeze_(0).to("cuda")  # (1, batch_size, after_token_num)
        return pred  # (batch_size, after_token_num)
